...
def test_network(image_path: str):
    ...
    nat_gateway.create_container(
        'gateway-test', image_path, [NetworkInterface(), NetworkInterface(vlan_tag=100, ip4='10.100.0.1/24')], onboot=1
    )
    ...
    dns_server.create_container(
        'dns-test', image_path, [NetworkInterface(vlan_tag=100, ip4='10.100.0.2/24', gw4='10.100.0.1')], onboot=1
    )
    ...
    dhcp_server.create_container(
        'dhcp-test', image_path, [NetworkInterface(vlan_tag=100, ip4='10.100.0.3/24', gw4='10.100.0.1')], onboot=1
    )
    dhcp_server.update_container()
    print(dhcp_server.get_ip(0))
    dhcp_service = DhcpService(dhcp_server, 'isc dhcpd service')
    dhcp_service.install(
        [
            Subnet(
...
def dns_master_and_slave(image_path: str):
    ...
    dns_master.create_container(
        'dns-test-master',
        image_path,
        # TODO: would be great if NetworkInterface could detect likely IP conflicts
        [NetworkInterface(vlan_tag=100, ip4='10.100.0.5/24', gw4='10.100.0.1')],
        onboot=1,
    )
    ...
    dns_slave.create_container(
        'dns-test-slave', image_path, [NetworkInterface(vlan_tag=100, ip4='10.100.0.6/24', gw4='10.100.0.1')], onboot=1
    )
    dns_slave.update_container()
    print(dns_slave.get_ip(0))

    dns_master_service = BindService(dns_master, 'dns master')
    dns_master_service.install_bind_dns_authoritative(
        dns_master.network_interfaces[0],
        master_zones=[MasterZone(domain_name='test.lan', slaves=[dns_slave.network_interfaces[0].ip4.ip])],
    )
    dns_slave_service = BindService(dns_slave, 'dns slave')
    dns_slave_service.install_bind_dns_authoritative(
        dns_slave.network_interfaces[0],
        slave_zones=[SlaveZone(domain_name='test.lan', masters=[dns_master.network_interfaces[0].ip4.ip])],
    )
...
def samba_server_and_client(image_path: str):
    ...
    samba_server.create_container(
        'samba-test',
        image_path,
        [NetworkInterface(mac='C2:25:0C:61:BF:F1', ip4='192.168.100.248/24', gw4='192.168.100.254')],
        onboot=1,
    )
    ...
    acme_service.install(
        acme_email=config.acme_email, ddns_server=config.ddns_server, ddns_tsig_key=config.ddns_tsig_key, staging=True
    )
    ...
    samba_service.install(
        ws=True, mdns=True, domain_master=True, ntlm_support=True, ldap_config=None, shares=[SAMBA_SHARE_HOMES]
    )
    ...
    samba_client.create_container(
        'samba-test2',
        image_path,
        [NetworkInterface(mac='C2:25:0C:61:BF:F2', ip4='192.168.100.247/24', gw4='192.168.100.254')],
        onboot=1,
    )
...
def nfs_server_and_client(image_path: str):
    ...
    nfs_server.create_container(
        'nfs-test',
        image_path,
        # [NetworkInterface(vlan_tag=100)],
        [NetworkInterface(mac='C2:25:0C:61:B1:01', ip4='192.168.100.245/24', gw4='192.168.100.254')],
        onboot=1,
        # unprivileged=0,
        # feature_mount='nfs',
        # feature_nesting=0,
    ...
    nfs_client.create_container(
        'nfs-test2',
        image_path,
        # [NetworkInterface(vlan_tag=100)],
        [NetworkInterface(mac='C2:25:0C:61:B1:02', ip4='192.168.100.244/24', gw4='192.168.100.254')],
        onboot=1,
        unprivileged=0,
        feature_mount='nfs',
        feature_nesting=0,
...
def transmission(image_path: str):
    ...
    transmission_server.create_container(
        'transmission-test',
        image_path,
        # [NetworkInterface(vlan_tag=100)],
        [NetworkInterface(mac='C2:25:0C:61:BF:4C', ip4='192.168.100.249/24', gw4='192.168.100.254')],
        onboot=1,
        unprivileged=0,
        feature_mount='nfs',
        feature_nesting=0,
...
def jellyfin(image_path: str):
    ...
    jellyfin_server.create_container(
        'jellyfin-test',
        image_path,
        # [NetworkInterface(vlan_tag=100)],
        [NetworkInterface(mac='C2:25:0C:61:BF:4D', ip4='192.168.100.246/24', gw4='192.168.100.254')],
        onboot=1,
        unprivileged=0,
        feature_mount='nfs',
        feature_nesting=0,
