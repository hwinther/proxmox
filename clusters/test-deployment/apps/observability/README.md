...
- Preloaded dashboards: OpenTelemetry Collector (15983), Collector data flow (18309). **ASP.NET Core (OTEL)** is a patched copy of grafana.com/19924 under folder **Custom** (variables use `http_server_request_duration_seconds_count` instead of `http_server_active_requests`, which is often absent). Import others from grafana.com as needed.
- If **Custom → ASP.NET Core (OTEL)** is still empty: in **Explore → Prometheus**, run `{__name__=~".*http.*server.*"}` or `http_server_request_duration_seconds_count` to confirm series and `job` / `instance` labels match the dropdowns; older SDKs may emit `http_server_duration_seconds_*` instead of `http_server_request_duration_seconds_*`.
