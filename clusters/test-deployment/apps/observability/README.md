...
- Preloaded dashboards: OpenTelemetry Collector (15983), Collector data flow (18309). **Custom** folder: **ASP.NET Core (OTEL)** (patched grafana.com/19924) and **CloudNativePG** (grafana.com/20417, Postgres metrics). Prometheus scrapes CNPG **`testdb`** pods in namespace **`test`** on port **`metrics`** (9187) via `extraScrapeConfigs` job **`cnpg-testdb`**.
