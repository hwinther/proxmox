...
Flux applies **`cnpg-system`** (operator HelmRelease) then **`cnpg-test`** (`Cluster` **`testdb`** in namespace **`test`**, `local-path` storage). **`test-api`** reads **`ConnectionStrings__Blogging`** from Secret **`testdb-app`** key **`uri`** (Npgsql). See [`../../production/apps/postgres-test/postgres-test-secrets.md`](../../production/apps/postgres-test/postgres-test-secrets.md).
