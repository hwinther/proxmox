...
- **Management plane:** **Homepage** uses **`mgmt.wsh.no`**. Other management UIs use **`{service}.mgmt.wsh.no`** — e.g. `grafana.mgmt.wsh.no`, `otel.mgmt.wsh.no`, `alertmanager.mgmt.wsh.no`, **`policy-reporter.mgmt.wsh.no`** (Kyverno reports via [Policy Reporter UI](https://kyverno.github.io/policy-reporter-docs/policy-reporter-ui/introduction.html)), **`redis-prod.mgmt.wsh.no`** / **`redis-test.mgmt.wsh.no`** (RedisInsight for **`shared-production`** / **`shared-test`** Valkey). Prefer stricter edge controls (WAF / auth) on `mgmt.wsh.no` and `*.mgmt.wsh.no`. **Authelia / OIDC issuer** is **`auth.wsh.no`** (public canonical URL; see `apps/authelia-production/authelia-helmrelease.yaml` and `infra/k0s/k0s.yaml.example` for API server OIDC `extraArgs`).
