...
| `PBS_RESTORE_SNAPSHOT` | Only for **restore**: full snapshot path (e.g. `host/clutterstock-test-sqlite/2025-04-13T10:00:00Z`). Omit for backup-only. You can add it temporarily, run the restore Job, then remove it or leave it unused. |
...
kubectl -n clutterstock-test patch secret clutterstock-test-pbs-backup \
  --type merge \
  -p '{"stringData":{"PBS_RESTORE_SNAPSHOT":"host/clutterstock-test-sqlite/2025-04-13T10:00:00Z"}}'
```
...
kubectl -n clutterstock-test create secret generic clutterstock-test-pbs-backup \
  ...
  --from-literal=PBS_RESTORE_SNAPSHOT='host/clutterstock-test-sqlite/2025-04-13T10:00:00Z'
```
