...
Middleware `otel-cors` in this namespace; allowlist includes `https://clutterstock.wsh.no` and `https://test.test.wsh.no`. **`accessControlAllowHeaders`** includes `Content-Type`, `Authorization`, and W3C trace headers (`traceparent`, `tracestate`, `baggage`) so browsers are not blocked from sending them on cross-origin OTLP POSTs to `otel.mgmt.wsh.no`.
...
- **Ingress-derived pod status** uses pod labels. For the OpenTelemetry ingress, **`gethomepage.dev/pod-selector`** must match the collector pods (e.g. `app.kubernetes.io/instance=obs-otel-collector`), not the Ingress resource name. The same applies when one Ingress backs several apps (e.g. Clutterstock UI + API): use a selector that matches all relevant pods, such as `app in (clutterstock-frontend, clutterstock-api)`.
